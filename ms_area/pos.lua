...
if core.parse_relative_number then
	...
	parse_relative_pos = function(x_str, y_str, z_str)
...
function areas:setPos1(name, pos)
	...
	if old_pos then
		...
		for _, object in ipairs(core.get_objects_inside_radius(old_pos, 0.01)) do
			...
			if luaentity and luaentity.name == "ms_area:pos1" and luaentity.player == name then
				object:remove()
			end
		end
	end

	local entity = core.add_entity(pos, "ms_area:pos1")
	if entity then
...
function areas:setPos2(name, pos)
	...
	if old_pos then
		...
		for _, object in ipairs(core.get_objects_inside_radius(old_pos, 0.01)) do
			local luaentity = object:get_luaentity()
			if luaentity and luaentity.name == "ms_area:pos2" and luaentity.player == name then
				object:remove()
			end
		end
	end

	local entity = core.add_entity(pos, "ms_area:pos2")
	if entity then
		local luaentity = entity:get_luaentity()
		if luaentity then
			luaentity.player = name
...
core.register_entity("ms_area:pos1", {
	initial_properties = {
		visual = "cube",
...
core.register_entity("ms_area:pos2", {
	initial_properties = {
		visual = "cube",
		visual_size = {x=1.1, y=1.1},
